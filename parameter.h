...
#define CLAD_HEIGHT1 2100	// 上部クラッド高さ
...
#define EXCT_LEN 777					// 励振点 (モデルの左端からの距離)
#define EXCT_OBSE_LEN 975+19*21				// 励振点から観測面の中心までの距離
#define OBSE_WIRE_LEN 1500 //2540				// 観測面の中心から細線導波路端までの距離
//#define OBSE_INTER (2 * PITCH)		// 観測面の長さ(透過率，反射率を求めるために使用)
#define OBSE_INTER (PITCH)				// 観測面の長さ(透過率，反射率を求めるために使用)
#define WIRE_OUTPUT_LEN EXCT_LEN+19*21 //(EXCT_LEN + 45)	// 出射細線導波路の長さ(直打ちの数字はセルサイズをNODE数の整数倍にするために使用)
...
#define WIRE_LEN2 (WIRE_LEN1-12*CELL_SIZE) //(OBSE_WIRE_LEN + WIRE_OUTPUT_LEN)		// 出射細線長
...
static const int Nmax = 1000000; 				// 最終時間ステップ
/*-------------------- CELL_SIZE:15nm --------------------*/
#endif
