...
#define ZMAX_FF ZMAX_ALL_FF
#define X_LSPCW (PITCH/CELL_SIZE*2)
...
double Ex[XMAX+1][YMAX+1][ZMAX_FF+1]; 
double Ey[XMAX+1][YMAX+1][ZMAX_FF+1]; 
double Ez[XMAX+1][YMAX+1][ZMAX_FF+1]; 
double Hx[XMAX+1][YMAX+1][ZMAX_FF+1]; 
double Hy[XMAX+1][YMAX+1][ZMAX_FF+1]; 
double Hz[XMAX+1][YMAX+1][ZMAX_FF+1]; 
...
double epsilonx[XMAX][YMAX+1][ZMAX_FF+1]; 
double epsilony[XMAX][YMAX+1][ZMAX_FF+1]; 
double epsilonz[XMAX][YMAX+1][ZMAX_FF+1]; 
int ALL_cell[XMAX_ALL+1][YMAX_ALL+1][ZMAX_ALL+1];		// 実際はALL_cell[XMAX_ALL+1][YMAX_ALL][ZMAX_ALL]で良いが，プログラムの都合上誘電体の配列と同じ次元にした
int cell[XMAX][YMAX+1][ZMAX_FF+1];							// 実際はcell[XMAX][YMAX][ZMAX]で良いが，プログラムの都合上誘電体の配列と同じ次元にした
double epsilon_xy[XMAX+1][YMAX+1], epsilon_yz[YMAX+1][ZMAX_FF+1], epsilon_zx[XMAX+1][ZMAX_FF+1]; 
double epsilon_zx2[XMAX+1][ZMAX_FF+1]; 
int cell_xy[XMAX][YMAX], cell_yz[YMAX][ZMAX_FF],cell_xz[XMAX][ZMAX_FF]; 

/*吸収境界条件適用のときに使う電界の配列*/
double Exn2y00[XMAX+1][ZMAX_FF+1], 	Exn1y00[XMAX+1][ZMAX_FF+1+1], 	Exn2y01[XMAX+1][ZMAX_FF+1], 	Exn1y01[XMAX+1][ZMAX_FF+1+1]; 
...
double Eyn2x00[YMAX][ZMAX_FF+1], 	Eyn1x00[YMAX+1][ZMAX_FF+1+1], 	Eyn2x01[YMAX][ZMAX_FF+1], 	Eyn1x01[YMAX+1][ZMAX_FF+1+1]; 
double Eyn2xm1[YMAX][ZMAX_FF+1], 	Eyn1xm1[YMAX+1][ZMAX_FF+1+1], 	Eyn2xm0[YMAX][ZMAX_FF+1], 	Eyn1xm0[YMAX+1][ZMAX_FF+1+1]; 

double Ezn2y00[XMAX+1][ZMAX_FF], 	Ezn1y00[XMAX+1][ZMAX_FF+1], 	Ezn2y01[XMAX+1][ZMAX_FF], 	Ezn1y01[XMAX+1][ZMAX_FF+1]; 
//double ezn2ym1[XMAX+1][ZMAX], 	ezn1ym1[XMAX+1][ZMAX+1], 	ezn2ym0[XMAX+1][ZMAX], 	ezn1ym0[XMAX+1][ZMAX+1]; 
double Ezn2x00[YMAX+1][ZMAX_FF], 	Ezn1x00[YMAX+1+1][ZMAX_FF+1], 	Ezn2x01[YMAX+1][ZMAX_FF], 	Ezn1x01[YMAX+1+1][ZMAX_FF+1]; 
double Ezn2xm1[YMAX+1][ZMAX_FF], 	Ezn1xm1[YMAX+1+1][ZMAX_FF+1], 	Ezn2xm0[YMAX+1][ZMAX_FF], 	Ezn1xm0[YMAX+1+1][ZMAX_FF+1]; 

// 磁界分布出力用
double field_xy[XMAX][YMAX]; 	// Hz-field のファイル出力 (面垂直方向の磁界成分)
double field_xz[XMAX][ZMAX_FF]; 	// Hz-field のファイル出力 (面垂直方向の磁界成分)
double field_yz[YMAX][ZMAX_FF];	// Hz-field のファイル出力 (面方向の磁界成分)

int x, y, z; 				// 離散座標
int xmax, ymax, zmax,zmax_ff; 			// 空間分割の最大値
int xmax_all, ymax_all, zmax_all,zmax_all_ff; //分割前の最大値
