...
int main(int argc, char **argv){
	...
	if (isize != ISIZE){
		//		printf ("MPI¤ÇÀßÄê¤·¤¿·×»»µ¡¤ÎÂæ¿?(%d)¤¬¥×¥úÁ°¥é¥àÃæ¤ÎÃÍ¤È°?Ã×¤·¤Þ¤»¤?¥\n½ªÎ»¤·¤Þ¤¹\n", ISIZE);
...
void file_open(char* dir_name_def){
	...
	//baba lab
	//_mkdir(strcpy(dir_name, dir_name_def)); 		// ?U?É?????????e?X?g

	//kuramitsu lab
	mkdir(strcpy(dir_name, dir_name_def), 0755); 		// ?U?É?????????e?X?g
	...
	model_xy = fopen (strcat(strcpy(dir_name, dir_name_def), name_xy), "w"); 		// ?U?É?????????e?X?g
